...
def export_session_markdown(session_id: str) -> Optional[str]:
    ...
    md.append(
        f"**Started:** {datetime.fromtimestamp(session['started_at']).strftime('%Y-%m-%d %H:%M')}"
    )
    if session.get("ended_at") and session["ended_at"]:
        md.append(
            f"**Ended:** {datetime.fromtimestamp(session['ended_at']).strftime('%Y-%m-%d %H:%M')}"
        )
    ...
    if scores:
        ...
        for s in scores:
            md.append(
                f"| {s.get('dimension', 'N/A')} | {s.get('score', 'N/A')} | {s.get('notes', '')} |"
            )
        md.append("")
    ...
    if insights:
        ...
        for i, insight in enumerate(insights, 1):
            md.append(
                f"{i}. **{insight.get('category', 'general')}**: {insight.get('content', '')}"
            )
        md.append("")
...
def export_all_sessions_markdown(user_id: str = "default") -> List[dict]:
    ...
    conn.row_factory = sqlite3.Row
    cur = conn.cursor()
    cur.execute("SELECT * FROM memory_sessions ORDER BY started_at DESC")
    ...
    return [
        {
            ...
            "started_at": datetime.fromtimestamp(s["started_at"]).strftime(
                "%Y-%m-%d %H:%M"
            ),
...
def get_public_session(share_id: str) -> Optional[dict]:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM memory_sessions WHERE session_id = ?", (share["session_id"],)
    )
    session = cur.fetchone()
    if not session:
        return None
...
def generate_rss_feed(base_url: str = "http://localhost:8773", limit: int = 20) -> str:
    ...
    for s in sessions:
        started = datetime.fromtimestamp(s["started_at"]).strftime(
            "%a, %d %b %Y %H:%M:%S GMT"
        )
        topic = (
            s["topic"].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        )
        ...
        rss.append(
            f"      <description>{topic} ({s['turn_count']} turns)</description>"
        )
        rss.append(f"    </item>")
